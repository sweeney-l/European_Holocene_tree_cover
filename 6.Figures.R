...
cop_masked_tree_cover_plot <- ggplotify::as.ggplot(~terra::plot(cop_tree_cover_masked, 
            ...
            axes = FALSE,
            mar = c(1,0,1,2),
            plg=list(cex = 0.75, size = c(0.75,1)))) # Legend text size
...
plot_bin_200_median_recon_tree_loc <- ggplot2::ggplot(data = dplyr::filter(bin_200_median_recon_tree_loc, bin_age <= 12000), mapping = aes(x = bin_age, y = median, color = half_width))+
  geom_line()+
  ylim(25,75)+
  theme_bw()+
  ylab("Median tree cover (%)")+
  ...
  scale_color_discrete(name = "Smoothing half-width", 
                       breaks = c("None", "500-year", "1000-year", "2000-year"))+
  scale_x_reverse(breaks = seq(12000, 0, -2000))+
  theme(axis.text=element_text(size=10, ),axis.title=element_text(size=10))+
  theme(text = element_text(family = "Times New Roman"), 
...
# Figure 6: Median tree cover values, for selected biogeographical regions. Smoothed lines reflect LOESS fitted regression with 1000-year halfwidth
...
ggsave("figs/6.binned_median_recon_bioregion_gr4.png", height = 6, width = 10, unit = "cm")



# Figure 7: Reconstructed median tree cover compared to equivalent extracted tree cover medians for Serge et al. (2023) and Zanon et al (2018). Smoothed lines reflect LOESS fitted regression with 1000-year halfwidth 
...
ggplot2::ggplot(data = dplyr::filter(comp_all_median_loc, half_width == "None" & source != "githumbi"), 
  ...
  theme(text = element_text(family = "Times New Roman"))+
  scale_x_reverse(breaks = seq(12000, 0, -2000))
ggsave("figs/7.comp_all_median_loc.png", height = 6, width = 10, unit = "cm")



# Figure 8: Reconstructed median tree cover compared to equivalent extracted tree cover medians for Serge et al. (2023) and Zanon et al (2018), for selected modern biogeographical regions. Smoothed lines reflect LOESS fitted regression with 1000-year halfwidth. 
#           A – Atlantic; 
#           B- Boreal; 
#           C- Continental; 
#           D – Mediterranean
...
ggpubr::ggarrange(plot_comp_all_median_loc_atlantic,plot_comp_all_median_loc_boreal,
                  ...
                  labels = c("A", "B", "C", "D"), hjust = -4.5, vjust = 3, font.label = list(family = "Times New Roman"))
ggsave("figs/8.bioregion_comparisons_median.png",height = 12, width = 16, unit = "cm")
...
# Supplementary Table 1: Higher level grouping of taxa considered native to Europe and included within the Total Terrestrial Pollen Sum
...
# Supplementary Figure 3: Extracted tree cover values averaged per gridcell for the first bin of Zanon et al (2018) compared to 
...
ggpubr::ggarrange(modern_obs_zanon_plot_ID,modern_pred_zanon_plot_ID,
                  ...
                  nrow = 2, ncol = 2,
                  labels = c("A", "B", "C", "D"), hjust = -4.5, vjust = 3, font.label = list(family = "Times New Roman"))
ggsave("figs/supplement/SF3.modern_comparisons_ID.png",height = 12, width = 16, unit = "cm")



# Supplementary Figure 4: A - Mean reconstructed tree cover for Europe from 12,000 to 0 cal. BP, with 95% confidence intervals for 1000 bootstrap resamples of records; 
...
plot_bin_200_mean_recon_tree_loc <- ggplot2::ggplot(data = filter(bin_200_mean_recon_tree_loc, bin_age <= 12000), mapping = aes(x = bin_age, y = mean, color = half_width))+
  geom_line()+
  ylim(25,75)+
  theme_bw()+
  ylab("Mean tree cover (%)")+
...
ggpubr::ggarrange(plot_binned_mean_recon_boots,plot_bin_200_mean_recon_tree_loc,
                  nrow = 1, ncol = 2,
                  labels = c("A", "B"), hjust = -4.5, vjust = 3, font.label = list(family = "Times New Roman"))
ggsave("figs/supplement/SF4.mean_tree_cover_plots.png",height = 6, width = 16, unit = "cm")
...
rio::export(data_binned_bioregion,"figs/supplement/ST3.data_binned_bioregion.csv")  


# Supplementary Figure 5: Median tree cover based on different the binwidths used in other vegetation reconstructions
...
ggplot2::ggplot(data = dplyr::filter(bin_200_median_recon_tree_plot, bin_age <= 12000),
  ...
  theme(axis.text=element_text(size=10, ),axis.title=element_text(size=10),legend.title=element_text(size=9), legend.text=element_text(size=8))+
  theme(text = element_text(family = "Times New Roman"))
ggsave("figs/supplement/SF5.otherreconbin_median.png",height = 6, width = 10, unit = "cm")


# Supplementary Figure 6: Median tree cover calculated using different fossil sample age model estimates
...
ggplot2::ggplot(data = dplyr::filter(bin_200_median_recon_tree_plot, bin_age <= 12000),
  ...
  theme(axis.text=element_text(size=10, ),axis.title=element_text(size=10),legend.title=element_text(size=9), legend.text=element_text(size=8))+
  theme(text = element_text(family = "Times New Roman"))
ggsave("figs/supplement/SF6.intqage_median.png",height = 6, width = 10, unit = "cm")



# Supplementary Figure 7: Median tree cover reconstructions, with reconstructions for Serge and Zanon data based ongridcell averages rather than record values
ggplot2::ggplot(data = dplyr::filter(comp_all_cell_median_loc, half_width == "None" & source != "githumbi"), 
  ...
  theme(text = element_text(family = "Times New Roman"))+
  scale_x_reverse(limits = c(12000, 0), breaks = seq(12000, 0, -2000))
ggsave("figs/supplement/SF7.comp_all_cell_median_loc.png", height = 6, width = 10, unit = "cm")





# Supplementary Figure 8: Median tree cover based on records within the Atlantic, Boreal, Continental and Mediterranean biogeogrpahical regions only 
ggplot2::ggplot(data = dplyr::filter(comp_all_bioregiongr4_median_EU_loc, half_width == "None" & source != "githumbi"), 
  ...
  theme(text = element_text(family = "Times New Roman"))+
  scale_x_reverse(breaks = seq(12000, 0, -2000))
ggsave("figs/supplement/SF8.comp_all_bioregiongr4_median_EU_loc.png", height = 6, width = 10, unit = "cm")
