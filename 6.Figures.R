...
bin_200_median_recon_tree_plot_ns <- rio::import("data/intermediate_output/vegetation/bin_200_median_recon_tree_plot_ns.csv")
...
bin_200_recon_tree_boots_5_95_median_ns <- rio::import("data/intermediate_output/vegetation/bin_200_recon_tree_boots_5_95_median_ns.csv")
bin_200_recon_tree_boots_ns <- rio::import("data/intermediate_output/vegetation/bin_200_recon_tree_boots_ns.csv")
...
epd_tree_shannon_bin200 <- rio::import("data/intermediate_output/vegetation/epd_tree_shannon_bin200.csv")
...
hengl_biomes_training_shannon_gr_long <- rio::import("data/intermediate_output/vegetation/hengl_biomes_training_shannon_gr_long.csv")
hengl_biomes_training_shannon_gr_summary1 <- rio::impolrt("data/intermediate_output/vegetation/hengl_biomes_training_shannon_gr_summary1.csv")
...
taxa_cat2 <- rio::import("data/input/taxa_cat2.csv")
tree_beta <- readRDS("data/intermediate_output/vegetation/tree_beta.rda") #modern tree cover model
tree_beta_50pol <- readRDS("data/intermediate_output/vegetation/tree_beta_50pol.rda") #modern tree cover model with different pollen source percentage
tree_beta_35pol <- readRDS("data/intermediate_output/vegetation/tree_beta_35pol.rda") #modern tree cover model with different pollen source percentage
tree_beta_lake <- readRDS("data/intermediate_output/vegetation/tree_beta_lake.rda") #modern tree cover model for lake only
tree_beta_not_crop <- readRDS("data/intermediate_output/vegetation/tree_beta_not_crop.rda") #store model with no crop exclusion in forest map
tree_beta_pinus <- readRDS("data/intermediate_output/vegetation/tree_beta_pinus.rda") #modern tree cover model with pinus rather than needleleaf %
...
tree_beta_wind <- readRDS("data/intermediate_output/vegetation/tree_beta_wind.rda") #modern tree cover model with pollination wind% rather than needleleaf
...
#Table 1: Key elements of the reconstruction technique from this study, the REVEALS approach (i.e. Serge et al., 2023) and MAT (i.e. Zanon et al., 2018)
#Manually cnostructed
...
# Table 2: Modern tree cover model coefficients
...
rio::export(list(results_table_mean,results_table_precision), "figs/2.model_results_table.xlsx")

# Table 3: Change in modern model AIC values when excluding specific variables (exclusion includes interactions, polynomials and precision variables)
...
rio::export(modern_AIC_values_change, "figs/3.model_AIC_table.xlsx")
...
taxa_cat_single2 <- taxa_cat2 %>% #for single clean_taxon_name
  dplyr::select(-taxon_name) %>% 
  dplyr::distinct()
taxa_cat_tps_native <- taxa_cat_single2 %>% 
...
taxa_cat_hp <- taxa_cat_tps_native %>% 
  ...
  dplyr::arrange(clean_taxon_name)
taxa_cat_wind <- taxa_cat_tps_native %>% 
  dplyr::filter(pollination == "wind") %>% 
  dplyr::arrange(clean_taxon_name)
taxa_cat_biotic <- taxa_cat_tps_native %>% 
  dplyr::filter(pollination == "biotic") %>% 
  dplyr::arrange(clean_taxon_name)
taxa_cat_dual <- taxa_cat_tps_native %>% 
  dplyr::filter(pollination == "dual") %>% 
  dplyr::arrange(clean_taxon_name)
...
taxa_cat_table <- tibble("Tree/Shrub/Non-arboreal" = c("Tree","Tree",
                         ...
                         "Tree sub-division" = c("Broadleaf", "Needleleaf",
                                                 "Wind pollinated", "Biotic pollination", "Dual wind and biotic pollination",
                                                 NA,NA,
                                                 NA,NA),
                         "Taxa grouping" = c(toString(c(taxa_cat_broad$clean_taxon_name)),toString(c(taxa_cat_needle$clean_taxon_name)),
                                             c(toString(c(taxa_cat_wind$clean_taxon_name)),toString(c(taxa_cat_biotic$clean_taxon_name)),toString(c(taxa_cat_dual$clean_taxon_name))),
...
# Supplementary Table 3: Modern tree cover model coefficients: wind pollination
results_model_wind <- summary(tree_beta_wind)
results_table_wind_mean <- dplyr::as_tibble(results_model_wind[["coefficients"]][["mean"]], rownames = "coefficients")
results_table_wind_precision <- dplyr::as_tibble(results_model_wind[["coefficients"]][["precision"]], rownames = "coefficients")
rio::export(list(results_table_wind_mean,results_table_wind_precision), "figs/supplement/ST3.model_results_wind_table.xlsx")


# Supplementary Table 4: Modern tree cover model coefficients: pinus
results_model_pinus <- summary(tree_beta_pinus)
results_table_pinus_mean <- dplyr::as_tibble(results_model_pinus[["coefficients"]][["mean"]], rownames = "coefficients")
results_table_pinus_precision <- dplyr::as_tibble(results_model_pinus[["coefficients"]][["precision"]], rownames = "coefficients")
rio::export(list(results_table_pinus_mean,results_table_pinus_precision), "figs/supplement/ST4.model_results_pinus_table.xlsx")


# Supplementary Table 5: Biome groups for included modelled modern records. Parenthesis indicates grouped biomes 
hengl_biomes_training_shannon_tab <- hengl_biomes_training_shannon_gr_summary1 %>% 
  dplyr::mutate(biome_group_name = dplyr::if_else(biome_group_name == "cool.evergreen.needleleaf.forest", 
                                                  "Cool evergreen needleleaf forest\n(Cold evergreen needleleaf forest; cool evergreen needleleaf forest)",
                                                  biome_group_name)) %>% 
  dplyr::mutate(biome_group_name = dplyr::if_else(biome_group_name == "tundra", 
                                                  "Tundra\n(Low and high shrub tundra; Erect dwarf shrub tundra)",
                                                  biome_group_name)) %>%
  dplyr::mutate(biome_group_name = dplyr::if_else(biome_group_name == "xerophytic.scrub.and.woodlands", 
                                                  "Xerophytic scrub and woodlands\n(Steppe; Xerophytic woods scrub)",
                                                  biome_group_name)) %>%
  dplyr::rename("Number of modern records" = count) %>% 
  dplyr::rename("Biome group\n (incl. Biomes)" = biome_group_name)  
rio::export(hengl_biomes_training_shannon_tab, "figs/supplement/ST5.hengl_biomes_training_shannon_tab.xlsx")


# Supplementary Figure 1: Boxplots of arboreal pollen and observed tree cover by biome group
ggplot2::ggplot(data = hengl_biomes_training_shannon_gr_long, mapping = aes(x = biome_group_name, y = Percentage*100, fill = Variable)) +
  geom_boxplot(position = position_dodge(width = 0.75)) +
  labs(x = "Biome group", y = "Percentage") +
  theme_bw()+
  scale_x_discrete(guide = guide_axis(n.dodge = 2),
                   labels = c("cool.evergreen.needleleaf.forest" = "Cool ever. needle. forest",
                              "cool.mixed.forest" = "Cool mixed forest",
                              "temperate.deciduous.broadleaf.forest" = "Temp. decid. broad. forest",
                              "tundra" = "Tundra",
                              "warm.temperate.evergreen.and.mixed.forest" = "Warm temp. ever. mixed forest",
                              "xerophytic.scrub.and.woodlands" = "Xero. scrub wood."))+
  scale_fill_viridis_d(option = "viridis",begin = 0.5, end = 0.9,
                       name = "",labels = c("ap_cover" = "Arboreal\npollen", "tree" = "Observed\ntree cover"))+
  theme(text = element_text(family = "Times New Roman"))+
  theme(axis.text=element_text(size=8),axis.title=element_text(size=10))
ggsave("figs/supplement/SF1.shannon_arb_obs.png", height = 6, width = 16, unit = "cm")


# Supplementary Figure 2: Boxplots of Shannon index values for tree pollen for each biome group
ggplot2::ggplot(data = hengl_biomes_training_shannon_gr, mapping = aes(y = tree_shannon, x = biome_group_name))+
  geom_boxplot()+
  labs(x = "Biome group", y = "Shannon index arboreal pollen") +
  theme_bw()+
  scale_x_discrete(guide = guide_axis(n.dodge = 2),
                   labels = c("cool.evergreen.needleleaf.forest" = "Cool ever. needle. forest",
                              "cool.mixed.forest" = "Cool mixed forest",
  ...
  theme(text = element_text(family = "Times New Roman"))+
  theme(axis.text=element_text(size=8),axis.title=element_text(size=10))
ggsave("figs/supplement/SF2.shannon_box.png", height = 6, width = 13, unit = "cm")


# Supplementary Table 6: Modern tree cover model coefficients: Lake sites
...
rio::export(list(results_table_lake_mean,results_table_lake_precision), "figs/supplement/ST6.model_results_lake_table.xlsx")


# Supplementary Table 7: Modern tree cover model coefficients: Including higher elevation sites* (n.b. run tree beta without elevation filter)
results_model <- summary(tree_beta)
results_table_mean <- dplyr::as_tibble(results_model[["coefficients"]][["mean"]], rownames = "coefficients")
results_table_precision <- dplyr::as_tibble(results_model[["coefficients"]][["precision"]], rownames = "coefficients")
rio::export(list(results_table_mean,results_table_precision), "figs/supplement/ST7.model_results_table.xlsx")

# Supplementary Table 8: Modern tree cover model coefficients: Stricter 500m limit* (n.b. run tree beta with 500m elevation filter)
results_model <- summary(tree_beta)
results_table_mean <- dplyr::as_tibble(results_model[["coefficients"]][["mean"]], rownames = "coefficients")
results_table_precision <- dplyr::as_tibble(results_model[["coefficients"]][["precision"]], rownames = "coefficients")
rio::export(list(results_table_mean,results_table_precision), "figs/supplement/ST8.model_results_table.xlsx")

#Supplementary Table 9: Modern tree cover model coefficients, without exclusion of crop dominated cells when calculating observed tree cover
results_model_not_crop <- summary(tree_beta_not_crop)
results_table_not_crop_mean <- dplyr::as_tibble(results_model_not_crop[["coefficients"]][["mean"]], rownames = "coefficients")
results_table_not_crop_precision <- dplyr::as_tibble(results_model_not_crop[["coefficients"]][["precision"]], rownames = "coefficients")
rio::export(list(results_table_not_crop_mean,results_table_not_crop_precision), "figs/supplement/ST9.model_results_not_crop_table.xlsx")

#Supplementary Table 10: Modern tree cover model coefficients, with 50% of pollen used to calculate source area rather than 70%
results_model_50pol <- summary(tree_beta_50pol)
results_table_50pol_mean <- dplyr::as_tibble(results_model_50pol[["coefficients"]][["mean"]], rownames = "coefficients")
results_table_50pol_precision <- dplyr::as_tibble(results_model_50pol[["coefficients"]][["precision"]], rownames = "coefficients")
rio::export(list(results_table_50pol_mean,results_table_50pol_precision), "figs/supplement/ST10.model_results_50pol_table.xlsx")

#Supplementary Table 11: Modern tree cover model coefficients, with 35% of pollen used to calculate source area rather than 70%
results_model_35pol <- summary(tree_beta_35pol)
results_table_35pol_mean <- dplyr::as_tibble(results_model_35pol[["coefficients"]][["mean"]], rownames = "coefficients")
results_table_35pol_precision <- dplyr::as_tibble(results_model_35pol[["coefficients"]][["precision"]], rownames = "coefficients")
rio::export(list(results_table_35pol_mean,results_table_35pol_precision), "figs/supplement/ST11.model_results_35pol_table.xlsx")

# Supplementary Figure 3: Model performance: 
...
ggpubr::ggarrange(pred_obs_scatter_plot, refit_obs_scatter_plot, pred_obs_plot, refit_obs_plot, nrow = 2, ncol = 2,
                  labels = c("A", "B", "C", "D"), hjust = -4.5, vjust = 3, font.label = list(family = "Times New Roman"))
ggsave("figs/supplement/SF3.predicted_refitted_obsevations_scatter_qmap.png", height = 12, width = 16, unit = "cm")



# Supplementary Figure 4: Spatial structure of differences between quantile mapped adjusted predictions and observations
...
ggsave("figs/supplement/SF4.map_differences_model.png",height = 10, width = 16, unit = "cm")


# Supplementary Figure 5: Arboreal pollen percentage compared to observed tree cover: 
...
ggpubr::ggarrange(ap_obs_scatter_plot, ap_obs_plot, nrow = 1, ncol = 2,
                  labels = c("A", "B", "C", "D"), hjust = -4.5, vjust = 3, font.label = list(family = "Times New Roman"))
ggsave("figs/supplement/SF5.ap_obsevations_scatter.png", height = 6, width = 16, unit = "cm")


# Supplementary Figure 6: Extracted tree cover values averaged per gridcell for the first bin of Zanon et al (2018) compared to 
...
ggpubr::ggarrange(modern_obs_zanon_plot_ID,modern_pred_zanon_plot_ID,
                  ...
                  nrow = 2, ncol = 2,
                  labels = c("A", "B", "C", "D"), hjust = -4.5, vjust = 3, font.label = list(family = "Times New Roman"))
ggsave("figs/supplement/SF6.modern_comparisons_ID.png",height = 12, width = 16, unit = "cm")



# Supplementary Figure 7: A - Mean reconstructed tree cover for Europe from 12,000 to 0 cal. BP, with 95% confidence intervals for 1000 bootstrap resamples of records; 
...
ggpubr::ggarrange(plot_binned_mean_recon_boots,plot_bin_200_mean_recon_tree_loc,
                  nrow = 1, ncol = 2,
                  labels = c("A", "B"), hjust = -4.5, vjust = 3, font.label = list(family = "Times New Roman"))
ggsave("figs/supplement/SF7.mean_tree_cover_plots.png",height = 6, width = 16, unit = "cm")

# Supplementary Figure 8: Median reconstructed tree cover, with model bootstraps
plot_boot_binned_median <- ggplot2::ggplot(data = dplyr::filter(boot_bin_200_median_recon_tree, bin_age <= 12000), 
  ...
  theme(axis.text=element_text(size=10, ),axis.title=element_text(size=10))+
  theme(text = element_text(family = "Times New Roman"))
ggsave("figs/supplement/SF8.plot_boot_binned_median.png",height = 6, width = 8, unit = "cm")


#Supplementary Figure 9: A - Median reconstructed tree cover for Europe from 12,000 to 0 cal. BP, with 95% confidence intervals for 1000 bootstrap resampling of records;
#                        B - Median reconstructed tree cover for Europe from 12,000 to 0 cal. BP excluding AP Shannon index, with 95% confidence intervals for 1000 bootstrap resampling of records
plot_binned_median_recon_boots <- ggplot2::ggplot(data = dplyr::filter(bin_200_recon_tree_boots, bin_age <= 12000), 
                                                  mapping = aes(y = tree_cover_median, x = bin_age, group = column_label))+
  geom_line(color = "light grey", size = 0.3)+
  geom_line(data = dplyr::filter(bin_200_median_recon_tree_plot, bin_age <= 12000),
...
plot_binned_median_recon_boots_ns <- ggplot2::ggplot(data = dplyr::filter(bin_200_recon_tree_boots_ns, bin_age <= 12000), 
                                                     mapping = aes(y = tree_cover_median, x = bin_age, group = column_label))+
  geom_line(color = "light grey", size = 0.3)+
  geom_line(data = dplyr::filter(bin_200_median_recon_tree_plot_ns, bin_age <= 12000),
            mapping = aes(y = tree_cover_median, x = bin_age), color = "yellow", size = 1)+
  geom_line(data = dplyr::filter(bin_200_recon_tree_boots_5_95_median_ns, bin_age <= 12000),
            mapping = aes(y = tree_cover_5_95, x = bin_age, group = lower_upper), color = "red",size = 0.2)+
  ylim(25,75)+
  theme_bw()+
  ylab("Median tree cover (%)")+
...
ggpubr::ggarrange(plot_binned_median_recon_boots,plot_binned_median_recon_boots_ns,
                  nrow = 1, ncol = 2,
                  labels = c("A", "B", "C"), hjust = -4.5, vjust = 3, font.label = list(family = "Times New Roman"))
ggsave("figs/supplement/SF9.median_tree_cover_plots_ns.png",height = 6, width = 16, unit = "cm")

# Supplementary Figure 10: AP Shannon index for Europe from 12,000 to 0 cal. BP, average for individual records binned in 200-year bins
epd_tree_shannon_bin200_plot <- ggplot2::ggplot(data = epd_tree_shannon_bin200, mapping = aes(x = bin_age, y = tree_shannon))+
  geom_point()+
  geom_smooth()+
  theme_bw()+
  theme(axis.text=element_text(size=10, ),axis.title=element_text(size=10), legend.title=element_text(size=9), legend.text=element_text(size=8))+
  theme(text = element_text(family = "Times New Roman"))+
  scale_x_reverse(limits = c(12000, 0), breaks = seq(12000, 0, -2000))+
  ylab("AP Shannon index")+
  xlab("Bin age (cal. BP)")
ggsave("figs/supplement/SF10.Shannon_binned_ns.png",epd_tree_shannon_bin200_plot,height = 6, width = 10, unit = "cm")


# SF11-SF19 - Gridded maps
#See "figs/tree_cover_rasters/"

# Supplementary Table 12: Data by biogeographical region
...
rio::export(data_binned_bioregion,"figs/supplement/ST12.data_binned_bioregion.csv")  


# Supplementary Figure 20: Median tree cover based on different the binwidths used in other vegetation reconstructions
ggplot2::ggplot(data = dplyr::filter(bin_200_median_recon_tree_plot, bin_age <= 12000),
  ...
  theme(axis.text=element_text(size=10, ),axis.title=element_text(size=10),legend.title=element_text(size=9), legend.text=element_text(size=8))+
  theme(text = element_text(family = "Times New Roman"))
ggsave("figs/supplement/SF20.otherreconbin_median.png",height = 6, width = 10, unit = "cm")


# Supplementary Figure 21: Median tree cover calculated using different fossil sample age model estimates
...
ggplot2::ggplot(data = dplyr::filter(bin_200_median_recon_tree_plot, bin_age <= 12000),
  ...
  theme(axis.text=element_text(size=10, ),axis.title=element_text(size=10),legend.title=element_text(size=9), legend.text=element_text(size=8))+
  theme(text = element_text(family = "Times New Roman"))
ggsave("figs/supplement/SF21.intqage_median.png",height = 6, width = 10, unit = "cm")


# Supplementary Figure 22: Median tree cover reconstructions, with reconstructions for Serge and Zanon data based ongridcell averages rather than record values
ggplot2::ggplot(data = dplyr::filter(comp_all_cell_median_loc, half_width == "None" & source != "githumbi"), 
  ...
  theme(text = element_text(family = "Times New Roman"))+
  scale_x_reverse(limits = c(12000, 0), breaks = seq(12000, 0, -2000))
ggsave("figs/supplement/SF22.comp_all_cell_median_loc.png", height = 6, width = 10, unit = "cm")



# Supplementary Figure 23: Median tree cover based on records within the Atlantic, Boreal, Continental and Mediterranean biogeogrpahical regions only 
ggplot2::ggplot(data = dplyr::filter(comp_all_bioregiongr4_median_EU_loc, half_width == "None" & source != "githumbi"), 
  ...
  theme(text = element_text(family = "Times New Roman"))+
  scale_x_reverse(breaks = seq(12000, 0, -2000))
ggsave("figs/supplement/SF23.comp_all_bioregiongr4_median_EU_loc.png", height = 6, width = 10, unit = "cm")
